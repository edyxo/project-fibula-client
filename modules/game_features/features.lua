...
controller:registerEvents(g_game, {
    onClientVersionChange = function(version)
        ...
        if version >= 770 then
            ...
            -- Project Fibula (world.fibula.app) manda stackpos en AddThing (0x6A) estilo TVP/Nostalrius.
            -- Sin esto el parser 7.72 se desalinea 1 byte -> invalid item id / no thing / no creature.
            g_game.enableFeature(GameTileAddThingWithStackpos)
            -- El server manda PingBack (op 29) de keepalive. Con GameClientPing off el cliente lo
            -- trata como respuesta huerfana -> spam "Got an invalid ping from server". Activandolo,
            -- ese 29 se enruta a processPing (responde, sin contador) -> sin error.
