...
function g_game.chooseRsa(host)
    if G.currentRsa ~= CIPSOFT_RSA and G.currentRsa ~= OTSERV_RSA and G.currentRsa ~= FIBULA_RSA then
        return
    end
    if host:ends('fibula.app') then
        -- Project Fibula: llave RSA custom (no la de OTServ)
